...
.close:focus {
 ...
 $i=$i+1;
		 ...
		 echo"<center><h1>YOUR SCORE IS :".$totalCorrect."/10</h1></center><br>";
			 ...
			 {echo "<center><h1>$row1[short_desp]</h1></center>";}
          $i=1;
          while($row2 = mysqli_fetch_array($retval1, MYSQLI_ASSOC))
          {   echo"<h3>Q".$i.") $row2[question]</h3>
              <h4 style='color:green'> Correct Ans : $row2[correct_ans]";echo ") ";
              $ans=$row2['correct_ans'];
              if($ans=="A")
              {echo "".$row2['option_A']."</h4>";}
            if($ans=="B")
              {echo "".$row2['option_B']."</h4>";}
            if($ans=="C")
              {echo "".$row2['option_C']."</h4>";}
            if($ans=="D")
              {echo "".$row2['option_D']."</h4>";}
          $i=$i+1;
          }
		 $conn -> close();
